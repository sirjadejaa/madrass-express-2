    ...
    <div className="fixed z-[100] flex flex-col gap-2 p-4 w-full md:max-w-[400px] left-1/2 -translate-x-1/2 md:left-auto md:translate-x-0 md:right-4 top-16 md:top-auto md:bottom-4 max-h-screen overflow-hidden pointer-events-none" style={{ width: "calc(100% - 32px)" }}>
      {toasts.map(function ({ id, title, description, action, variant, open, ...props }) {
        ...
        return (
          <div 
            key={id} 
            className={`pointer-events-auto flex w-full items-start justify-between space-x-4 overflow-hidden rounded-xl border p-4 shadow-lg transition-all duration-250 ease-out ${
